...
class UserAdapter(private val listUser: SearchResponse) : RecyclerView.Adapter<UserAdapter.UserViewHolder>() {
    ...
    override fun onBindViewHolder(holder: UserViewHolder, position: Int) {
        Glide.with(holder.itemView.context)
            .load(listUser.items[position].avatarUrl)
            .apply(RequestOptions.circleCropTransform())
            .into(holder.imgPhoto)
        holder.tvName.text = listUser.items[position].login
        holder.itemView.setOnClickListener {
           val intent = Intent(holder.itemView.context, DetailActivity::class.java)
            intent.putExtra(EXTRA_USERNAME, listUser.items[position].login)
            holder.itemView.context.startActivity(intent)
        }
    }

    override fun getItemCount(): Int = listUser.items.size
